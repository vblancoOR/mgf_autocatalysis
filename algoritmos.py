...
# =============================================================================
def growthRateinSubgraphAlt2(output_matrix, input_matrix, t_max):
    ...
    def modelGrowthRateFixed(alpha0):
        ...
        m.addConstrs(
            ...
            alpha <= gb.quicksum(output_matrix[s, r] * x[r] 
                    ...
                    - alpha0 * gb.quicksum(input_matrix[s, r] * x[r]
                                            for r in reactions) 
                    + alpha0*sum(input_matrix[s,:])*upper_bound*(1 - a[s])
                    for s in species),
            name = "name1")
        #
        # m.addConstrs(
        ...
        if m.status != gb.GRB.OPTIMAL:
            ...
            return xsol, alphasol, asol, zsol
        # --------------------------------------
    # =========================================================================
    ...
    start=time.time()
    while stop == False:
        xx, alphabar, aa, zz = modelGrowthRateFixed(alpha)
        ...
        if (len(aa) < 1 or 
            ...
            return xx, alpha, step, alphaDict, aa, zz, time.time()-start
        else:
            alphaDict[step] = alpha
            alpha_old = alpha
            step += 1
...
# =============================================================================
def growthRateinSubgraphAlt1(output_matrix, input_matrix, t_max):
...
def growthRateGraph(output_matrix, input_matrix, max_steps):
    ...
    while stop == False:
        ...
        if (np.abs(alphabar) < 0.00000001 or
            step > max_steps
            or np.abs(alpha_old - alpha_t) <  0.0000001):
            stop = True
...
def growthRateWithTime(output_matrix, input_matrix, t_max, number_periods):
    ...
    def growthRateWithTimeFixed(alpha_0):
        ...
        # m.addConstrs(
        #     (y[s, t] <= 
        #      gb.quicksum(z[r, t] for r in reactions 
        #                  if input_matrix[s,r] > 0
